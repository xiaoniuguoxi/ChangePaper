...
namespace ChangePaper
{
    public partial class Form1 : Form
    {
        ...
        private void btn_changeResult_Click(object sender, EventArgs e)
        {
            ...
            //打开文件夹
            System.Diagnostics.Process.Start("Explorer.exe", odaTemp);
        }

        private void btn_browserMp3_Click(object sender, EventArgs e)
        {
            OpenFileDialog ofd = new OpenFileDialog();
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                txt_mp3.Text = ofd.FileName;
            }
        }

        private void btn_mp3ToWav_Click(object sender, EventArgs e)
        {
            if (File.Exists(txt_mp3.Text.Trim()))
            {
                FileInfo fi = new FileInfo(txt_mp3.Text);
                {
                    string ext = fi.Name.Replace(".mp3", ".wav");
                    ChangeResultPack.Main.ToWav(Path.Combine(fi.Directory.FullName, ext), txt_mp3.Text.Trim());
                    //打开文件夹
        ...
        private void btn_spoken_brw_result_Click(object sender, EventArgs e)
        {
            OpenFileDialog ofd = new OpenFileDialog();
            ofd.Filter = "结果包文件(*.cdp)|*.cdp;";
            ofd.ValidateNames = true;
            ofd.CheckPathExists = true;
            ofd.CheckFileExists = true;
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                fileResult = ofd.FileName;
                txt_spoken_result.Text = fileResult;
                fileSign = fileResult.Replace(".cdp", ".ckp");
                if (File.Exists(fileSign))
                {
                    txt_spoken_sign.Text = fileSign;
                }
                StringBuilder sb = new StringBuilder(fileResult);
                StringBuilder co = new StringBuilder();
                SecModule.GetFileSignMD5(sb, co);
            }
        }

        private void btn_spoken_brw_sign_Click(object sender, EventArgs e)
        {
            OpenFileDialog ofd = new OpenFileDialog();
            ofd.Filter = "结果包签名文件(*.ckp)|*.ckp;";
            ofd.ValidateNames = true;
            ofd.CheckPathExists = true;
            ofd.CheckFileExists = true;
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                fileSign = ofd.FileName;
                txt_spoken_sign.Text = fileSign;
                fileResult = fileSign.Replace(".ckp", ".cdp");
                if (File.Exists(fileSign))
                {
                    txt_spoken_result.Text = fileResult;
                }
        ...
        private void btn_spoken_result_change_Click(object sender, EventArgs e)
        {
            string odaTemp = "";
            ChangeResultPack.Main.SpokenOpen(txt_spoken_result.Text.Trim(), txt_spoken_sign.Text.Trim(), out odaTemp);

            //打开文件夹
            System.Diagnostics.Process.Start("Explorer.exe", odaTemp);
        }

        private void btn_spokenPaper_Click(object sender, EventArgs e)
        {
            OpenFileDialog ofd = new OpenFileDialog();
            ...
            {
                filePaper = ofd.FileName;
                txt_spokenPaper.Text = filePaper;
                fileCode = filePaper.Replace(".sjp", ".code");
                if (File.Exists(fileCode))
                {
                    txt_spokenCode.Text = fileCode;
                }
            }
        }

        private void btn_spokenCode_Click(object sender, EventArgs e)
        {
            OpenFileDialog ofd = new OpenFileDialog();
        ...
        {
            FileInfo fi = new FileInfo(txt_spoken_result.Text.Trim());

            ChangeResultPack.Main.SpokenEncrypt(fi.Directory.FullName);
